...
class Courses(tag: Tag) extends Table[Course](tag, "courses") {
  def courseId = column[Option[Int]]("course_id", O.PrimaryKey, O.AutoInc)
...
object Course {
  def create(schoolId: SchoolId, departmentId: Int, departmentSpecificCourseId: String, name: String): Course = {
    Course(None, schoolId.id, departmentId, departmentSpecificCourseId, name)
  }
}
